...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    x_norm1 = torch.sum(x_1 ** 2, dim=1).reshape(-1, 1)
    x_norm2 = torch.sum(x_2 ** 2, dim=1)
    distances = torch.exp((-1) * (x_norm1 + x_norm2 - 2*torch.matmul(x_1, x_2.T)) / (2 * sigma**2))   ### YOUR CODE HERE
